...
Такой двойной механизм действия делает KEAP1 актуальной и научно обоснованной мишенью.

### 3. Доступность данных

*   **Доступность данных по активности:** В ChEMBL существует готовый датасет (`CHEMBL2069156`) для IC50 с примерами высокоактивных лигандов. Это единственный датасет, удовлетворящий требованиям (Организм:человек, тип: одиночный белок). Этот датасет используется для обучения предиктора.
*   **Доступность структурных данных:** Для молекулярного докинга доступна кристаллическая структура комплекса KEAP1 в PDB (`ID: 4L7B`) с разрешением  2.41 Å.
...
Наш проект построен на **взаимодействии двух принципиально разных, но взаимодополняющих архитектур**, каждая из которых была выбрана для решения своей специфической задачи: одна — для **предсказания (оценки)**, другая — для **генерации (создания)**.

### 1. Архитектура Предсказательной Модели
**Задача:** Создать максимально точную и надежную функцию, которая по структуре молекулы сможет предсказать ее биологическую активность (pIC50). Эта модель будет направлять наш генератор в процессе обучения с подкреплением.
...
*   **Признаковое Представление (Features):** Мы не подаем SMILES напрямую. Молекула сначала превращается в числовой вектор, состоящий из:
    1.  **Отпечатков Моргана (Morgan Fingerprints):** Кодируют наличие определенных циклических подструктур в молекуле.
...
*   **Ансамбль Моделей и Стэкинг (Stacking):** Мы устроили "конкурс" между алгоритмами (`RandomForest`, `XGBoost`, `LightGBM`) и объединили их с помощью **`StackingRegressor`**. Этот метод ансамблирования создает мета-модель, которая обучается на предсказаниях базовых моделей, что позволяет получить итоговый предиктор, который точнее и стабильнее, чем любая из моделей по отдельности.

### 2. Архитектура Генеративной Модели
**Задача:** Создать модель, которая может создавать новые, химически корректные, разнообразные и высокоактивные молекулы, обучаясь на обратной связи от предиктора (предсказательной модели).

**Выбранная Архитектура и Обоснование:**
*   **Представление Молекул (SELFIES):** Мы использовали **SELFIES** — строковое представление, которое по своей грамматике **гарантирует 100% валидность** любой сгенерированной строки. 
...
*   **Стратегия Обучения (Предобучение + RL):**
    ...
    2.  **Тонкая настройка через RL:** Затем уже обученная модель доучивается решать конкретную задачу, используя оценки от нашего предиктора в качестве награды.
...
**2. Концептуальная Проблема: "Обман функции вознаграждения"**
...
Мы провели эксперимент с готовым инструментом **DiffSBDD** для генерации молекул в активном центре KEAP1. DiffSBDD — SE(3)-эквивариантная диффузионная модель, которая генерирует новые лиганды с учётом белковых карманов. Эта модель была выбрана как одна из передовых в своей области. Применив к его результатам нашу систему оценки, мы подтвердили, что наш кастомный RL-пайплайн обеспечивает **лучший контроль над многопараметрической оптимизацией** (активность, селективность, ADMET), что критически важно для сложных задач.
...
*   **Инструмент:** Мы использовали **GNINA**, которая применяет CNN для улучшения функции оценки и работает с 3D структурами.
*   **Процесс:** Лучшие молекулы были "пристыкованы" к активному центру KEAP1 (`PDB ID: 4L7B`).
*   **Результат:** Докинг послужил **финальным независимым фильтром**, подтвердив, что наши кандидаты не только обладают хорошими расчетными свойствами, но и способны к аффиному взаимодействию в кармане связывания мишени.
