...
Этот репозиторий содержит полный набор инструментов для генерации, оценки и валидации новых молекулярных ингибиторов белка KEAP1. Проект разделен на три независимых, но взаимосвязанных приложения, предназначенных для решения задач в рамках терапии болезни Альцгеймера. Генерация молекул осуществляется двумя способами: с помощью интеграции существующей модели DiffSBDD и реализацией собственной RL-модели.
...
- **DiffSBDD**: Эквивариантная диффузионная модель для разработки лекарств на основе структуры.
...
### 3. Обоснование выбора ввиду In Silico пайплайна
Помимо высокой научной актуальности, выбор KEAP1 продиктован доступностью данных:
...
*   **Доступность структурных данных:** Для молекулярного докинга доступна кристаллическая структура комплекса KEAP1 в PDB (`ID: 4L7B`) с высоким разрешением.
...
### 1. Архитектура Предсказательных Моделей
...
*   **Ансамбль Моделей и Стэкинг (Stacking):** Мы устроили "конкурс" между мощными алгоритмами (`RandomForest`, `XGBoost`, `LightGBM`) и объединили их с помощью **`StackingRegressor`**. Этот метод ансамблирования создает мета-модель, которая обучается на предсказаниях базовых моделей, что позволяет получить итоговый предиктор, который точнее и стабильнее, чем каждая модель по отдельности.

### 2. Архитектура Генеративной Модели
**Задача:** Создать модель, которая может создавать новые, химически корректные, разнообразные и высокоактивные молекулы, обучаясь на обратной связи от предсказательной модели.

**Выбранная Архитектура и Обоснование:**
*   **Представление Молекул (SELFIES):** Мы не использовали SMILES. Вместо этого мы выбрали **SELFIES** — строковое представление, которое по своей грамматике **гарантирует 100% валидность** любой сгенерированной строки. Это обеспечивает уверенность в том, что наш генератор всегда создает осмысленные структуры.
